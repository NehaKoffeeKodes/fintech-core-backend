...
class AdminBannerView(APIView):
    ...
    def add_new_Adminbanner(self, request):
        try:
            ...
            if not client_name:
                return Response({'status': 'fail', 'message': 'client_name is required.'}, status=400)
    ...
    def put(self, request):
        try:
            ...
            if not banner_id or not is_positive_integer(banner_id):
                return Response({'status': 'fail', 'message': 'Valid banner_id is required.'}, status=400)

            banner = Adminbanner.objects.filter(id=banner_id, is_deleted=False).first()
            if not banner:
                return Response({'status': 'fail', 'message': 'Adminbanner not found.'}, status=404)
            ...
            if new_name and Adminbanner.objects.filter(client_name__iexact=new_name, is_deleted=False).exclude(id=banner_id).exists():
                return Response({'status': 'fail', 'message': 'Another Adminbanner with this client name already exists.'}, status=400)
            ...
            def handle_photo_upload(files):
                ...
                for file in files:
                    ...
                    if ext not in {'png', 'jpg', 'jpeg', 'webp'}:
                        return Response({'status': 'fail', 'message': 'Invalid image format.'}, status=400)
            ...
            if not any(k in request.data for k in ['client_name', 'designation', 'review_text', 'rating', 'profile_photo']):
                ...
                if active_count >= 5 and not banner.is_inactive:
                    return Response({'status': 'fail', 'message': 'Maximum 5 active Adminbanners allowed.'}, status=400)
                if active_count <= 1 and banner.is_inactive == False:
                    return Response({'status': 'fail', 'message': 'At least one Adminbanner must remain active.'}, status=400)
            ...
            return Response({'status': 'success', 'message': msg}, status=200)
        ...
        try:
            with transaction.atomic():
                ...
                if not banner_obj.is_inactive and active_count <= 1:
                    return Response({'status': 'fail', 'message': 'Cannot delete the last active Adminbanner.'}, status=400)
                ...
                return Response({'status': 'success', 'message': 'Adminbanner removed successfully.'}, status=200)
    ...
    def get(self, request):
        try:
            ...
            if not banners.exists():
                return Response({'status': 'fail', 'message': 'No Adminbanners found.', 'data': []}, status=404)
