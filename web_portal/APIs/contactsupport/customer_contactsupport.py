...
class ContactSupportView(APIView):
    ...
    def _list_tickets(self, request):
        try:
            ...
            if page_size < 1 or page_no < 1:
                return Response({'status': 'fail', 'message': 'Invalid pagination.'}, status=400)
    ...
    def put(self, request):
        try:
            ...
            if not ticket_id or not new_status:
                return Response({'status': 'fail', 'message': 'ticket_id and status required.'}, status=400)
            ...
            return Response({'status': 'fail', 'message': 'Ticket not found.'}, status=404)
        except Exception as e:
            return Response({'status': 'error', 'message': str(e)}, status=500)
    ...
    def delete(self, request):
        try:
            ticket_id = request.data.get('ticket_id')
            if not ticket_id:
                return Response({'status': 'fail', 'message': 'ticket_id required.'}, status=400)
...
class AddContactSupportView(APIView):
    ...
    def post(self, request):
        try:
            ...
            return Response({'status': 'error', 'message': 'Service temporarily down.'}, status=500)